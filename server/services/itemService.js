...
const UrlApiML = 'https://api.mercadolibre.com';
...
function getItemDescription(id) {
    var request = {
        url: `${UrlApiML}/items/${id}/description`,
        method: 'GET'
    };
...
function getItems(query) {
    ...
    var request = {
        url: `${UrlApiML}/sites/MLA/search?q=${query}`,
        method: 'GET'
    };
    ...
    .then(response => {
        ...
        if (data.results) {
            ...
            //slice del inicio al final sin inlcluir
            items = data.results.slice(0,4);

            items = items.map(item => {
                var aux_price = item.price.toString().split('.');
                const price = aux_price[0];
                const decimals = aux_price[1] ? aux_price[1] : '00';
